...
port_file_path = "/var/tmp/lsp_port_session_abc.pid"
...
class RepoEnv(gym.Env):
    def __init__(self,
                 ...
                 use_lsp: bool = False):
        # Get the logger
        print("!!! now in ./R2E-Gym/src/r2egym/agenthub/environment/env.py")
        if logger is None:
        ...
        # assert (
        #     'ip' in args.ds
        # ), f"Invalid dataset format: input dataset {args.ds['instance_id']} must have *ip* attribute!!"
        # print("=ds="*20)
        # print(f"!!!!!!!!!!!!!!!!!!!Very import, env_ds:{args.ds}")
        if "ip":
            self.ip = ip
        elif "ip" in args.ds:
            self.ip = args.ds['ip']
        else:
            self.logger.warning("No ip input, please make sure you docker connection is right!")
        print("=self_ip="*20)
        print(f"!!!!!!!!!!!!!!!!!!!Very import, self_ip:{self.ip}")
        self.use_lsp = use_lsp
        self.runtime = DockerRuntime(
            ds=args.ds, command=["/bin/bash", "-l"], backend=backend, logger=self.logger, ip=self.ip, use_lsp = self.use_lsp
        )
    ...
    def reset(self) -> Dict[str, Any]:
        ...
        self.runtime = DockerRuntime(
            ds=self.args.ds, command=["/bin/bash", "-l"], logger=self.logger, backend=self.backend, ip=self.ip
        )
    ...
    # currently hard code, can be added to config yaml lately

    def initialize_lsp(self):
        # for lsp tools
        self.logger.info(f"started initialize_lsp")
        # -------
        # start the lsp daemon

        bash_output, error_code = self.runtime.run("env | sort > /tmp/exec_env.txt")
        self.logger.info(f"run for env | sort > /tmp/exec_env.txtoutput:{bash_output}, error_code:{error_code}")
        bash_output, error_code = self.runtime.run("cat /tmp/exec_env.txt")
        self.logger.info(f"run for cat /tmp/exec_env.txt output:{bash_output}, error_code:{error_code}")
        bash_output, error_code = self.runtime.run("which pyright")
        self.logger.info(f"run for cat which pyright output:{bash_output}, error_code:{error_code}")
        bash_output, error_code = self.runtime.run("which node")
        self.logger.info(f"run for cat which node output:{bash_output}, error_code:{error_code}")
         

        bash_output, error_code = self.runtime.run("nohup lsp_daemon > /usr/local/bin/daemon_stdout.log 2> /usr/local/bin/daemon.log &")
        self.logger.info(f"run for nohup lsp daemon started output:{bash_output}, error_code:{error_code}")

        time.sleep(10)
        bash_output, error_code = self.runtime.run("cat /usr/local/bin/daemon.log")
        self.logger.info(f"run for cat /usr/local/bin/daemon.log output: \n {bash_output}, \n error_code: \n {error_code}")
        
        # retry mechanism
        if "LSP server initialized successfully" not in bash_output:
            
            bash_output, error_code = self.runtime.run("lsp_tool daemon_shutdown")
            self.logger.info(f"run for lsp_tool daemon_shutdown output:{bash_output}, error_code:{error_code}")
            time.sleep(40)

            bash_output, error_code = self.runtime.run("nohup lsp_daemon > /usr/local/bin/daemon_stdout.log 2> /usr/local/bin/daemon.log &")
            self.logger.info(f"run for RETRY lsp daemon started output:{bash_output}, error_code:{error_code}")
            time.sleep(10)

        bash_output, error_code = self.runtime.run("cat /usr/local/bin/daemon.log")
        self.logger.info(f"run for cat /usr/local/bin/daemon.log output: \n {bash_output}, \n error_code: \n {error_code}")

        # bash_output, error_code = self.runtime.run("lsp_tool get_workspace_symbols --query run")
        # self.logger.info(f"run for RETRY lsp_tool output:{bash_output}, error_code:{error_code}")
        
        # bash_output, error_code = self.runtime.run("cat /usr/local/bin/daemon.log")
        # self.logger.info(f"run for cat /usr/local/bin/daemon.log output: \n {bash_output}, \n error_code: \n {error_code}")


        bash_output, error_code = self.runtime.run("cat /usr/local/bin/daemon_stdout.log")
        self.logger.info(f"run for cat /usr/local/bin/daemon_stdout.log output: \n {bash_output},\n error_code: \n {error_code}")

        # # /-- for debug  --/
        # bash_output, error_code = self.runtime.run("ls /usr/local/bin")
        # self.logger.info(f"run for ls /usr/local/bin output: \n {bash_output},  \n error_code: \n {error_code}")
        # bash_output, error_code = self.runtime.run("ls /var/tmp")
        # self.logger.info(f"run for ls /var/tmp output: \n {bash_output}, \n error_code: \n {error_code}")

        # time.sleep(60)
        # bash_output, error_code = self.runtime.run("lsp_tool get_definition --file_path /testbed/sympy/parsing/tests/test_sympy_parser.py --line 9 --character 25")
        # self.logger.info(f"run for lsp tool output: \n {bash_output}, error_code: \n {error_code}")
        # bash_output, error_code = self.runtime.run("ls /usr/local/bin")
        # self.logger.info(f"After run for ls /usr/local/bin output: \n {bash_output},  \n error_code: \n {error_code}")
        # bash_output, error_code = self.runtime.run("ls /var/tmp")
        # self.logger.info(f"After run for ls /var/tmp output: \n {bash_output}, \n error_code: \n {error_code}")
        # bash_output, error_code = self.runtime.run("cat /usr/local/bin/daemon.log")

        # self.logger.info(f"After run for cat /usr/local/bin/daemon.log output: \n {bash_output}, \n error_code: \n {error_code}")
        # bash_output, error_code = self.runtime.run("cat /usr/local/bin/daemon_stdout.log")
        # self.logger.info(f"After run for cat /usr/local/bin/daemon_stdout.log output: \n {bash_output},\n error_code: \n {error_code}")
        # # /-- end   --/
    ...
    def step(
        ...
        # check for lsp_tool
        if 'lsp_tool' in action.to_bashcmd():
            bash_output, error_code = self.runtime.run("ls /var/tmp")
            self.logger.info(f"After run for ls /var/tmp output: \n {bash_output}, \n error_code: \n {error_code}")
            bash_output, error_code = self.runtime.run("cat /var/tmp/lsp_port_session_abc.pid")
            self.logger.info(f"After run for cat /var/tmp/lsp_port_session_abc.pid output: \n {bash_output}, \n error_code: \n {error_code}")

            bash_output, error_code = self.runtime.run("cat /usr/local/bin/daemon.log")
            self.logger.info(f"After run for cat /usr/local/bin/daemon.log output: \n {bash_output}, \n error_code: \n {error_code}")
            bash_output, error_code = self.runtime.run("cat /usr/local/bin/daemon_stdout.log")
            self.logger.info(f"After run for cat /usr/local/bin/daemon_stdout.log output: \n {bash_output},\n error_code: \n {error_code}")
            bash_output, error_code = self.runtime.run("ps aux | grep lsp_daemon")
            self.logger.info(f"After run for ps aux | grep lsp_daemon output: \n {bash_output},\n error_code: \n {error_code}")
            bash_output, error_code = self.runtime.run("ps -ef | grep lsp_daemon")
            self.logger.info(f"After run for ps -ef | grep lsp_daemon output: \n {bash_output},\n error_code: \n {error_code}")
